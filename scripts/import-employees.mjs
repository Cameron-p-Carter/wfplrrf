...
async function main() {
  ...
  const csvPath = resolve(__dirname, '../supabase/migrations/employee_details.csv')
  ...
  // --- Step 2: import people (manager_id resolved in step 3) ---
  console.log('\nImporting people...')
  ...
  for (let i = 0; i < active.length; i += BATCH) {
    ...
    for (const r of batch) {
      ...
      if (!role_type_id) {
        console.warn(`  Skipping ${r.display_name}: no role_type for position "${r.employee_position}"`)
        skipped++
        continue
      }
      records.push({
    ...
    if (error) {
      ...
      process.stdout.write(`  ${imported}/${active.length - skipped} imported\r`)
    }
  }

  console.log(`\nImported ${imported}, skipped ${skipped}`)

  // --- Step 3: resolve manager_id via external_hr_id ---
  ...
  for (const person of allPeople) {
    ...
    if (error) {
      console.warn(`  Could not set manager for ${person.id}:`, error.message)
    } else {
      resolved++
    }
  }

  console.log(`  Resolved: ${resolved}, unresolved (manager not in active import): ${unresolved}`)
  console.log('\nDone.')
}
